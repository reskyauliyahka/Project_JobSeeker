@extends('pelamar.templates.master')

@section('content')
<div id="editUser" class="w-1/2 mx-auto p-6 bg-blue-100 border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 mb-2" style="margin-top: 120px">
    <form action="{{ route('updateUser', ['id' => Auth::user()->id]) }}" method="POST" >
        <h1 class="text-lg font-bold text-center">Edit Pengguna</h1>
        @csrf
        @method('PUT')
        <div
            class="max-w-full p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 mb-2">
            <div class="flex-1 mt-2">
                <label for="name" class="text-md font-bold">Username</label>
                <input type="text" id="name" name="name" value="{{ $user->name }}"
                    class="w-full p-2 mt-2 rounded-xl">
            </div>

            <!-- Email -->
            <div class="flex-1 mt-2">
                <label for="email" class="text-md font-bold">Email</label>
                <input type="text" id="email" name="email" value="{{ $user->email }}"
                    class="w-full p-2 mt-2 rounded-xl">
            </div>

            <!-- Tombol Submit -->
            <div class="mt-8 flex justify-center max-w-full ">
                <a href="{{route('pelamar.show', ['id' => Auth::id()])}}"
                    class="no-underline font-bold text-white bg-blue-200 hover:bg-blue-100 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800"
                    >
                    Batal
                </a>
                <button type="submit"
                    class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">
                    Simpan
                </button>
            </div>
        </div>
    </form>
</div>
    
@endsection